...
func _on_pressed():
	CharacterHandler.currentCharacter = CharacterHandler.Character.LYVOK
	CharacterHandler.currentCharacterSwitched.emit()
	
	## TEMP
	acceleration_field.set_text(str(TestCharacterStats.lyvok_acceleration))
	max_speed_field.set_text(str(TestCharacterStats.lyvok_max_speed))
	jump_field.set_text(str(TestCharacterStats.lyvok_jump_power))
	brake_field.set_text(str(TestCharacterStats.lyvok_brake_power))
