...
func _on_pressed():
	CharacterHandler.currentCharacter = CharacterHandler.Character.DJ
	CharacterHandler.currentCharacterSwitched.emit()
	
	## TEMP
	acceleration_field.set_text(str(TestCharacterStats.dj_acceleration))
	max_speed_field.set_text(str(TestCharacterStats.dj_max_speed))
	jump_field.set_text(str(TestCharacterStats.dj_jump_power))
	brake_field.set_text(str(TestCharacterStats.dj_brake_power))
